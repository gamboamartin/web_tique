...
class dp_municipio_htmlTest extends test {
    ...
    /**
     */
    public function test_select_dp_municipio_id(): void
    {
        errores::$error = false;
        $_GET['session_id'] = 1;
        $_GET['seccion'] = 'dp_estado';
        $html = new html();
        $dir = new dp_municipio_html($html);

        $cols = 1;
        $con_registros = false;
        $id_selected = -1;
        $resultado = $dir->select_dp_municipio_id($cols, $con_registros, $id_selected, $this->link);
        ...
        $this->assertIsString($resultado);
        $this->assertNotTrue(errores::$error);
        $this->assertStringContainsStringIgnoringCase("-sm-1'><label class='control-label' for='dp_municipio_id'>Municipio</label><div class='controls'><select",$resultado);
