...
use html\cat_sat_tipo_jornada_nom_html;
...
class controlador_cat_sat_tipo_jornada_nom extends system {

    public function __construct(PDO $link, html $html = new \gamboamartin\template_1\html(),
                                stdClass $paths_conf = new stdClass()){

        $modelo = new cat_sat_tipo_jornada_nom(link: $link);
        $html_ = new cat_sat_tipo_jornada_nom_html(html: $html);
        $obj_link = new links_menu($this->registro_id);
        parent::__construct(html:$html_, link: $link,modelo:  $modelo, obj_link: $obj_link, paths_conf: $paths_conf);

        $this->titulo_lista = 'Tipo jornada';
